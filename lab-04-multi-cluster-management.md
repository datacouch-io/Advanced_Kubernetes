# Lab 4 — Connecting and Managing Multiple Clusters Across Cloud Providers

**Day 1 · Multi-Cluster & Service Mesh**

> Every command below was actually run end to end against the same real GKE, EKS, and AKS clusters from Lab 3, and every screenshot is a real `screencapture` of that run — including the full teardown at the end.
...
- Why fleet **attachment** (Lab 3) and Kubernetes **RBAC** are two separate layers, both of which you need.
- A practical pattern for running the same operation across every cluster in the fleet from one terminal.

## Time & cost

- **Time:** ~30-40 minutes.
- **Cost:** none beyond what Lab 3 is already spending — this lab reuses those three clusters. **This is where you tear everything down** (§4.5).

## Prerequisites

**[Lab 3](lab-03-gke-fleet-attached-clusters.md), completed, in the same sitting.** You need `advk8s-gke`, `advk8s-eks`, and `advk8s-aks` all provisioned and attached to your fleet, with `kubectl` contexts named accordingly.

---

## 4.1 The problem: three clouds, three ways to authenticate
...
```mermaid
flowchart LR
    subgraph BEFORE["Before: 3 tools, 3 credential types"]
        direction TB
        U1["You"] -->|"gcloud identity"| GKE1["advk8s-gke"]
        U1 -->|"AWS IAM"| EKS1["advk8s-eks"]
        U1 -->|"Azure AD"| AKS1["advk8s-aks"]
    end

    subgraph AFTER["After: Connect Gateway"]
        direction TB
        U2["You (gcloud identity only)"] --> CGW["connectgateway.googleapis.com"]
        CGW -->|"Forbidden until RBAC granted<br/>(generate-gateway-rbac --apply)"| GKE2["advk8s-gke"]
        CGW -->|"Forbidden until RBAC granted"| EKS2["advk8s-eks"]
        CGW -->|"Forbidden until RBAC granted"| AKS2["advk8s-aks"]
    end

    BEFORE -.->|"fleet attachment collapses this"| AFTER
```

## 4.2 Connect Gateway: one identity, any fleet member
...
![Forbidden -- fleet attachment alone doesn't grant Kubernetes RBAC](screenshots/lab03-04/07-connect-gateway-403.png)
...
![The generated RBAC ClusterRole/ClusterRoleBindings, applied to the AKS cluster](screenshots/lab03-04/08-connect-gateway-aks-success.png)
...
![EKS reached through Connect Gateway after the RBAC grant](screenshots/lab03-04/09-connect-gateway-eks-success.png)
...
Full evidence: [`evidence/lab04-connect-gateway-aks.txt`](evidence/lab04-connect-gateway-aks.txt) and [`evidence/lab04-full-fleet-connect-gateway.txt`](evidence/lab04-full-fleet-connect-gateway.txt).
...
## 4.3 One control point, three clouds
...
![One credential, three clouds, one loop -- all real](screenshots/lab03-04/10-unified-fleet-loop.png)
...
One credential, three clouds, one loop. Full data: [`evidence/lab04-unified-multicloud-loop.txt`](evidence/lab04-unified-multicloud-loop.txt).
...
- **A service mesh spanning the fleet** — the natural next step after Labs 5 and 6, extending mTLS and traffic policy across cluster boundaries, not just within one cluster.

If your organization's actual need is "run the same Deployment reliably in 5 clusters with per-cluster overrides," revisit [Lab 2 §B.2](lab-02-cluster-api-kubefed-federation.md) — that's precisely the KubeFed problem statement, and Karmada or OCM (linked there) are the maintained tools for it today.

---

## 4.5 Teardown — do this now

You've been running three live, billed clusters since Lab 3. Delete them in this order (attached-cluster unregistration first, so the fleet's records stay clean, then the underlying clusters):
...
![All four checks empty: 0 fleet memberships, 0 GKE clusters, no EKS clusters, no AKS clusters, advk8s-rg gone](screenshots/lab03-04/12-teardown-verified.png)

The same result is visible in the Cloud Console (Kubernetes Engine → Clusters) — an empty clusters list for the project:

![Google Cloud Console: Kubernetes clusters list, empty](screenshots/lab03-04/11-console-clusters-empty.png)
...
- Screenshots: [`screenshots/lab03-04/`](screenshots/lab03-04/) (13 images, shared with Lab 3 — same three clusters, one continuous run)
- Logs: [`evidence/lab04-connect-gateway-aks.txt`](evidence/lab04-connect-gateway-aks.txt), [`evidence/lab04-full-fleet-connect-gateway.txt`](evidence/lab04-full-fleet-connect-gateway.txt), [`evidence/lab04-unified-multicloud-loop.txt`](evidence/lab04-unified-multicloud-loop.txt)

**Day 1 continues with:** [Lab 5 — Deploying Istio and configuring traffic shaping, retries, and circuit breaking](lab-05-istio-traffic-shaping.md).
