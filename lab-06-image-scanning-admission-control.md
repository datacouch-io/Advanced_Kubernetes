# Lab 6 — Setting Up Image Scanning and Admission Control (Kyverno)
...
## 6.1 A note on tool choice, and why the policy syntax below may look unfamiliar
...
## 6.2 Create the cluster and install Kyverno
...
## 6.3 Scan images with Trivy first
...
![166 vulnerabilities in nginx:1.16 (Debian 10.3)](screenshots/lab06/01-trivy-old-image.png)

```bash
trivy image --severity HIGH,CRITICAL --format table nginx:1.27-alpine
```

![35 vulnerabilities in the current image -- fewer, never zero](screenshots/lab06/02-trivy-new-image.png)
...
## 6.4 Policy 1: reject `:latest` and untagged images
...
![bad-pod denied by disallow-latest-tag; good-pod created](screenshots/lab06/03-disallow-latest-tag.png)

---

## 6.5 Policy 2: require non-root and resource limits — and the trap waiting on the other side
...
![good-pod-2 admitted, then crashes](screenshots/lab06/04-admitted-but-crashes.png)
...
![Confirmed: mkdir() "/var/cache/nginx/client_temp" failed (13: Permission denied)](screenshots/lab06/05-crash-confirmed.png)
...
![good-pod-3, same policy-compliant securityContext, different image: Running](screenshots/lab06/06-fixed-running.png)
...
## 6.6 Policy 3: disallow privileged containers
...
![Denied by both policies at once](screenshots/lab06/07-disallow-privileged.png)
...
Full evidence for this whole lab: [`evidence/lab06-image-scanning-admission-control.txt`](evidence/lab06-image-scanning-admission-control.txt).

---

## 6.7 Optional: see the policies enforcing live, in a GUI
...
![Headlamp events: ValidatingPolicy require-nonroot-and-limits flagging kube-system/headlamp pods for missing CPU/memory limits](screenshots/lab06/08-headlamp-policy-events.png)
...
![Cluster deleted](screenshots/lab06/09-cleanup.png)
...
- Screenshots: [`screenshots/lab06/`](screenshots/lab06/) (9 images)
- Logs: [`evidence/lab06-image-scanning-admission-control.txt`](evidence/lab06-image-scanning-admission-control.txt)

**Next:** [Lab 7 — Configuring GKE Workload Identity Federation and Binary Authorization](lab-07-gke-workload-identity-binary-authorization.md).
