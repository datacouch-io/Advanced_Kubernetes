...
This is the part that actually matters for tying back to [Lab 5](lab-05-istio-traffic-shaping.md): a retry policy with a `perTryTimeout` shorter than this real, measured (~4s) delay, and enough attempts to reach a replica outside its jittered delay window, is the kind of config decision this experiment makes provable instead of theoretical — the same distinction Lab 5 drew between fault-injection theater and a retry policy validated against a real, injected failure. The number to design that timeout against, per this run, is ~4s, not the ~2s the YAML alone would suggest.
